...
def extract_tripinfo_vfr():
    ...
    for tripinfo in root.findall('tripinfo'):
        ...
        if value is not None and float(value) <= 14440.0:
            ...
            underscore_index = car_departure.index('_')
            if (car_departure[:underscore_index] not in depart_list):
                depart_list.append(car_departure[:underscore_index])
            car_departure = car_departure[:-1]
            car_arrival= tripinfo.get('arrivalLane')
            underscore_index = car_arrival.index('_')
            if (car_arrival[:underscore_index] not in arrive_list):
                arrive_list.append(car_arrival[:underscore_index])
    ...
    if (depart_list.sort() == all_depart.sort()):
        print("True")
    if(arrive_list.sort() == all_arrive.sort()):
        print("True")                
