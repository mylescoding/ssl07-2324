...
def extract_tripinfo(attribute_name, flag = 0):
    ...
    for tripinfo in root.findall('tripinfo'):
        ...
        if value is not None and flag == 1:
            if float(tripinfo.get('time')) == float(14400.0):
...
def extract_summary(attribute_name, flag = 0):
    ...
    for step in root2.findall('step'):
        ...
        if value is not None:
            values.append(float(value))
        elif value is not None and flag == 1:
            timestep = step.get('time')
            print(timestep)
            print("haha")
            if timestep == "14400.00":
...
print(f"Average queue length (veh) - whole simulation: " + str(average_qt_whole/24))

average_qt_4hr = extract_summary(attribute_qt,  1)
print(f"Average queue length (veh) - 4 hour: " + str(average_qt_4hr/24))
...
average_vfr = extract_summary(attribute_vfr, 0)
