...
def addGenericOptions(argParser):
    ...
    argParser.add_argument("--time-to-teleport", dest="timetoteleport", type=argParser.time, default=-1,
    ...
    argParser.add_argument("--departlane", action="store", dest="departlane",
                           default="free", help="sets the departure lane")
    argParser.add_argument("--departspeed", action="store", dest="departspeed",
                           default="desired", help="sets the departure speed")
    argParser.add_argument("--departpos", action="store", dest="departpos",
                           default="last", help="sets the departure position?")
...
def initOptions():
    ...
    argParser.add_argument("--lateral-resolution", type=float, default=0.2,
                           help="sets the lateral resolution")
...
def writeRouteConf(duarouterBinary, step, options, dua_args, file,
    ...
    args = [
        ...
        #'--departlane', str(options.departlane),
        #'--departspeed', str(options.departspeed),
    ]
    ...
    if options.departlane:
        args += ['--departlane', options.departlane]
    if options.departspeed:
        args += ['--departspeed', options.departspeed]
        if options.departpos:
            args += ['--departpos', options.departpos]
...
def writeSUMOConf(sumoBinary, step, options, additional_args, route_files):
    ...
    sumoCmd = [sumoBinary,
               ...
               '--lateral-resolution', options.lateral_resolution,
               ] + additional_args
    ...
    if options.mesosim:
        ...
        if options.lateral_resolution is not None:
            sumoCmd += ["--lateral-resolution", str(options.lateral_resolution)]
